...
global procedure w32store( atom struct, sequence s, object o )
    ...
    if      datatype = Byte then
        ...
        if atom(o) then
            ...
            l_SafePoke(where, o[1 .. lCnt])
        end if

    elsif   datatype = Word then
        -- poke a word
            bytes = int_to_bytes( o )
            l_SafePoke( where, bytes[1 .. 2] )
        ...
        if atom( o ) then
            ...
            l_SafePoke4( where, w32acquire_mem( struct, o  ) )
        end if
        ...
        for i = 1 to lCnt do
            l_SafePoke(where, atom_to_float32(o[i]))
            where += 4
        end for
        ...
        for i = 1 to length(o) do
            l_SafePoke(where, atom_to_float64(o[i]))
            where += 8
        end for
...
global function w32fetch( atom struct, sequence s )
    ...
    if      size = Byte then
        ...
        at = l_SafePeek4u( l_SafePeek4u( at ) )
        return at
        ...
        if lNumData[2] = 0 then
            ...
            if lPos > 0 then
                ...
                if length(lValue) > 1 and
                        (lValue[1] = '{' and lValue[length(lValue)] = '}')
                then
